...
export default function NearbyCarparks() {
  ...
  const handlePlaceChanged = () => {
    // safe access (in case Autocomplete not ready)
    const place = autocompleteRef.current?.getPlace?.();
    if (place && place.geometry) {
      const newLoc = {
        lat: place.geometry.location.lat(),
  ...
  // If API key missing, show a clear message
  if (!apiKey) {
    return (
      <div className="map-container">
        <div style={{ padding: 20 }}>
          ...
          <p>
            Set <code>VITE_GOOGLE_MAPS_API_KEY</code> in your <code>.env</code>{" "}
  ...
  return (
    <LoadScript googleMapsApiKey={apiKey} libraries={["places"]} version="weekly">
      <div className="map-container">
        <div className="top-buttons">
          <button className="back-home-btn" onClick={() => navigate("/home")}>
            ← Back to Home
